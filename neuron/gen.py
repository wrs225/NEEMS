...
class NeuronModel(MixedSignalModel):
    def __init__(
        ...
        V_out_st = self.add_analog_state("V_out_st", init=-0.07, range_=0.1)
        ...
        self.set_this_cycle(digital_sel, V_out_st <= float(VT))
        ...
        self.set_next_cycle(
            ...
            if_(
                ...
                + float(dt)
                * (
                    float(g_soma) * (float(EL) - V_out_st)
                    + float(g_soma)
                    * float(DeltaT)
                    * ((V_out_st - float(VT)) / float(DeltaT))
                    * ((V_out_st - float(VT)) / float(DeltaT))
                    + I_in
                )
                * (1 / float(C_soma)),
...
class NeuronModel(MixedSignalModel):
    def __init__(
        ...
        I_in = self.add_analog_input("I_in")
        V_out_st = self.add_analog_state("V_out_st", init=-70, range_=100)
        V_out = self.add_analog_output("V_out")
        self.set_this_cycle(V_out, V_out_st)
        ...
        self.set_next_cycle(
            ...
            if_(
                ...
                * (
                    ...
                    + float(g_soma * 1000)
                    * float(DeltaT * 1000)
                    * ((V_out_st - float(VT * 1000)) / float(DeltaT))
                    * ((V_out_st - float(VT * 1000)) / float(DeltaT))
                    + I_in
                )
                * (1 / float(C_soma * 1000)),
